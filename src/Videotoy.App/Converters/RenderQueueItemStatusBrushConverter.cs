...
public sealed class RenderQueueItemStatusBrushConverter : IValueConverter
{
    ...
    private static readonly SolidColorBrush RunningBrush = new(Color.FromRgb(0x3D, 0x8B, 0xE0));
    private static readonly SolidColorBrush NeutralBrush = new(Color.FromRgb(0x8A, 0x8A, 0x8A));

    public object Convert(object value, Type targetType, object parameter, CultureInfo culture) =>
        value switch
        {
            RenderQueueItemStatus.Succeeded => SuccessBrush,
            RenderQueueItemStatus.Failed => ErrorBrush,
            RenderQueueItemStatus.Running => RunningBrush,
            _ => NeutralBrush
        };
